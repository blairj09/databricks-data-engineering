...
class TestWeatherDataQuality:
    ...
    def test_temperature_reasonable_range(self, load_fixture):
        """Test that temperatures are within reasonable range."""
        weather_data = load_fixture("sample_weather.json")
    ...
    def test_humidity_valid_range(self, load_fixture):
    ...
    def test_precipitation_non_negative(self, load_fixture):
    ...
    def test_weather_category_present(self, load_fixture):
    ...
    def test_is_raining_consistent_with_rain(self, load_fixture):
        ...
        for record in weather_data:
            ...
            if rain > 0:
                assert is_raining == True, "is_raining should be True when rain > 0"
            else:
                assert is_raining == False, "is_raining should be False when rain = 0"


class TestTripDataQuality:
    """Tests for taxi trip data quality checks."""

    def test_fare_non_negative(self, load_fixture):
        """Test that fares are non-negative."""
        trips_data = load_fixture("sample_trips.json")

        for trip in trips_data:
            fare = trip.get("fare_amount")
            assert fare >= 0, f"Fare {fare} is negative"

    def test_trip_distance_non_negative(self, load_fixture):
    ...
    def test_pickup_before_dropoff(self, load_fixture):
    ...
    def test_required_fields_present(self, load_fixture):
...
class TestJoinDataQuality:
    ...
    def test_join_coverage(self, load_fixture):
